...
public class AmazonS3BucketService {
	...
	public void createBucket(@RequestBody HashMap<String, String> userMap) {
		try {
	...
	public List<UserDocDVO> getDocs(@RequestParam("folderName") String folderName) {
	...
	public String uploadDocs(@RequestParam("file") MultipartFile f) throws IOException {
		Authentication auth = SecurityContextHolder.getContext().getAuthentication();
		NVaultUser user = (NVaultUser) auth.getPrincipal();
		S3Bucket bucket = bucketService.findByuserName(user.getUsername());
